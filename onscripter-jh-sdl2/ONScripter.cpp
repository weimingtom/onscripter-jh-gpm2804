...
if (IMG_Init(IMG_INIT_JPG | IMG_INIT_PNG | IMG_INIT_TIF | IMG_INIT_WEBP) < 0) {
        utils::printError("Couldn't initialize SDL_image: %s\n", SDL_GetError());
        exit(-1);
}
#endif
...
#if defined(PSP) || defined(IPODLINUX) || defined(GP2X) || defined(WINCE) || defined(GCWZERO) || defined(MIYOO)
...
#if defined(MIYOO)
    window = SDL_CreateWindow(NULL, window_x, window_y, 480, 640, window_flag);
#else
...
#if defined(MIYOO)
    SDL_RenderSetLogicalSize(renderer, 480, 640); //Auto scale center
#else
    SDL_RenderSetLogicalSize(renderer, screen_width, screen_height); //Auto scale center
#endif
